...
| 🗂️ **Missing Indexes** | Tables with high sequential scan rates vs index usage |
| 📦 **Storage / Bloat** | Dead rows (Postgres), fragmentation (MySQL), large collections (Mongo) |
| ⚙️ **Resource Usage** | Buffer cache hit rate, connection counts, lock contention, memory, credits |
